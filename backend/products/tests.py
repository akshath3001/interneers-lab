...
class ProductAPITestCase(APITestCase):
    ...
    def test_get_single_product(self):
        response = self.client.post('/products/', self.product1, format='json')
        prod=Product.objects.filter(product_name__iexact=self.product1["product_name"]).first()
    ...
    def test_create_invalid_product(self):
        ...
        self.assertEqual(response.data["product_name"][0], "This field may not be blank.")
        prod["product_price"]=-300
        response = self.client.post('/products/', prod, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["product_price"][0], "Product price should be greater than 0")
